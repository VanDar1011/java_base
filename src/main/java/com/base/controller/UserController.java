...
public class UserController {
    ...
    public ResponseEntity<ResponseObject> findAll(@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "4") int size, @RequestParam(required = false) String name) {
        ...
        return ResponseEntity.status(HttpStatus.OK).body(new ResponseObject(ResponseStatus.FAIL.getStatus(), "FINDING SUCCESS", userService.getAll(paginationRequest)));
    }
    ...
    public ResponseEntity<ResponseObject> findById(@PathVariable("id") int id) {
        ...
        return ResponseEntity.status(HttpStatus.OK).body(new ResponseObject(ResponseStatus.OK.getStatus(), "Finding by ID success", user));
    }
